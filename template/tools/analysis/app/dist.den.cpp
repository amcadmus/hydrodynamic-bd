...
int main (int argc, char * argv[])
{
  ...
  desc.add_options()
      ...
      ("output,o",  po::value<std::string > (&ofile)->default_value (std::string("dist.den.out"), "output of density distribution"));
  ...
  while (tl.load()){
    ...
    if ((nCountFrame-1) % 1000 == 0){
      printf ("# dt: %f load frame at time %f       \r", double(dt), tl.getTime());
      fflush (stdout);
    }
	...
	// avgs[idx].deposite (1.0);
	tmpavg[idx] += 1.0;
      }
    }
    double volume = hbin * box[0] * box[1];
    for (unsigned ii = 0; ii < unsigned(nbin); ++ii){
      avgs[ii].deposite (tmpavg[ii] / volume);
    }
  }
  printf ("\n");
  ...
  for (int ii = 0; ii < nbin; ++ii){
    ...
    fprintf (fpo, "%f  %e  %e\n", xx, avgs[ii].getAvg(), avgs[ii].getAvgError());
  }
  fclose (fpo);
