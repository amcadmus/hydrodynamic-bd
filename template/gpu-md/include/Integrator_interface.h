...
class Langevin_BAOAB
{
  ...
  ScalorType sigma;
  ScalorType dt;
  // t0 and t1 is bathed to T0
  ScalorType T0;
  TypeType t0;
  TypeType t1;
  ScalorType pref0, pref1;
  ScalorType invsqrt3;
  // use the host way to generate random numbers, maybe not the best...
  IndexType nrandom;
  ScalorType * random_x;
  ScalorType * random_y;
  ScalorType * random_z;
  curandGenerator_t gen_x ;
  curandGenerator_t gen_y ;
  curandGenerator_t gen_z ;
  bool initilized;
  void free ();
public:
  Langevin_BAOAB ()
      : initilized (false) {}
  Langevin_BAOAB (const MDSystem & sys,
		  const ScalorType & dt,
		  const ScalorType & gamma,
		  const ScalorType & T0,
		  const TypeType & t0,
		  const TypeType & t1,
		  const IndexType & NThread,
      ...
      : initilized (false)
      {reinit (sys, dt, gamma, T0, t0, t1, NThread, seed);}
  ~Langevin_BAOAB ();
  void reinit (const MDSystem & sys,
	       const ScalorType & dt,
	       const ScalorType & gamma,
	       ...
	       const IndexType & NThread,
	       unsigned long long seed = 1);
  void stepAOA (MDSystem & sys, MDTimer * timer = NULL);
  void stepB (MDSystem & sys, MDTimer * timer = NULL);
  void stepB (MDSystem & sys, MDStatistic & st, MDTimer * timer = NULL);
}
    ;
