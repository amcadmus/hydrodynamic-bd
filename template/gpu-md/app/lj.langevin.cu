...
//  sysTop.addNonBondedInteraction (Topology::NonBondedInteraction(0, 0, ljparam_ww));
  ...
  Langevin_BAOAB langevin (sys, dt, gamma, refT, 0, 0, NThreadsPerBlockAtom, seed);
...
//	tfremover.remove (sys, &timer);
      ...
      langevin.stepB (sys, &timer);
      langevin.stepAOA (sys, &timer);
	...
	printf ("%09d %07e %.7e %.7e %.7e %.7e %.7e %.7e\n",
		...
		st.nonBondedEnergy(),
		st.kineticEnergy(),
		st.kineticEnergy() * 2. / 3. / (double (num_f+num_w) - 3.),
		st.nonBondedEnergy() +
		st.kineticEnergy(),
		st.nonBondedEnergy() +
		st.kineticEnergy() +
