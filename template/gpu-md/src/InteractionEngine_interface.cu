...
void InteractionEngine::clearInteraction (MDSystem & sys)
{
  ...
  clearForce
      <<<atomGridDim, myBlockDim>>>(
	  sys.ddata.numAtom,
	  sys.ddata.recordForcx, sys.ddata.recordForcy, sys.ddata.recordForcz);
...
applyNonBondedInteraction  (MDSystem & sys,
			    ...
			    const InteractionType & recordType,
			    ...
			    MDTimer *timer )
{
  ...
  calNonBondedInteraction_neighbor
	  ...
	  sys.ddata.forcz,
	  recordType,
	  sys.ddata.recordForcx,
	  sys.ddata.recordForcy,
	  sys.ddata.recordForcz,	    
	  sys.ddata.type, 
	  sys.box,
	  nlist.dnlist);
  }
...
calNonBondedInteraction_neighbor (const IndexType		numAtom,
				  ...
				  ScalorType *			forcy, 
				  ScalorType *			forcz,
				  const IndexType		recordForceIndex,
				  ScalorType *			recordForcx,
				  ScalorType *			recordForcy, 
				  ScalorType *			recordForcz,				  
				  const TypeType *		type,
				  const RectangularBox		box,
				  const DeviceNeighborList	nlist)
{
  ...
  ScalorType recordFsumx = 0.0f;
  ScalorType recordFsumy = 0.0f;
  ScalorType recordFsumz = 0.0f;
  ...
  if (ii < numAtom) {
    ...
    for (IndexType jj = 0, nlistPosi = ii;
      ...
      fsumy += fy;
      fsumz += fz;
      if (recordForceIndex == nbForceIndex){
	recordFsumx += fx;
	recordFsumy += fy;
	recordFsumz += fz;
      }
    ...
    forcy[ii] += fsumy;
    forcz[ii] += fsumz;
    recordForcx[ii] += recordFsumx;
    recordForcy[ii] += recordFsumy;
    recordForcz[ii] += recordFsumz;
  }
}
