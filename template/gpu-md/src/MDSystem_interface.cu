			   ...
			   const IndexType & maxNumAtom)
{
  ...
  for (IndexType i = 0; i < hdata.numAtom; ++i){
    ...
    hdata.recordForcx[i] = 0.f;
    hdata.recordForcy[i] = 0.f;
    hdata.recordForcz[i] = 0.f;
  }
...
writeHostDataTrr (int step, float time, MDTimer *timer)
{
  ...
  for (IndexType i = 0; i < hdata.numAtom; ++i){
    ...
    xdtrrf[i][0] = hdata.recordForcx[i];
    xdtrrf[i][1] = hdata.recordForcy[i];
    xdtrrf[i][2] = hdata.recordForcz[i];
  }
  xdbox[0][0] = box.size.x;
  xdbox[1][1] = box.size.y;
  xdbox[2][2] = box.size.z;
...
recoverDeviceData (MDTimer * timer)
{
  ...
  Reshuffle_reshuffleArray
      <<<atomGridDim, myBlockDim>>>
      (ddata.recordForcx, ddata.numAtom, backMapTable, recoveredDdata.recordForcx);
  Reshuffle_reshuffleArray
      <<<atomGridDim, myBlockDim>>>
      (ddata.recordForcy, ddata.numAtom, backMapTable, recoveredDdata.recordForcy);
  Reshuffle_reshuffleArray
      <<<atomGridDim, myBlockDim>>>
      (ddata.recordForcz, ddata.numAtom, backMapTable, recoveredDdata.recordForcz);
  Reshuffle_reshuffleArray
      <<<atomGridDim, myBlockDim>>>
	   ...
	   MDTimer * timer)
{
  ...
  Reshuffle_reshuffleArray
      <<<atomGridDim, myBlockDim>>>
      (bkDdata.recordForcx, ddata.numAtom, indexTable, ddata.recordForcx);
  Reshuffle_reshuffleArray
      <<<atomGridDim, myBlockDim>>>
      (bkDdata.recordForcy, ddata.numAtom, indexTable, ddata.recordForcy);
  Reshuffle_reshuffleArray
      <<<atomGridDim, myBlockDim>>>
      (bkDdata.recordForcz, ddata.numAtom, indexTable, ddata.recordForcz);
  Reshuffle_reshuffleArray
      <<<atomGridDim, myBlockDim>>>
