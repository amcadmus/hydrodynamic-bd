...
HostMDData::HostMDData()
{
  ...
  recordForcx = recordForcy = recordForcz = NULL;
  type = NULL;
  mass = massi = NULL;
  charge = NULL;
  atomName = NULL;
				...
				HostMDData * hdata)
{
  ...
  hdata->recordForcx = (ScalorType *) malloc (sizef);
  if (hdata->recordForcx == NULL) throw MDExcptFailedMallocOnHost("hdata->recordForcx", sizef);
  hdata->recordForcy = (ScalorType *) malloc (sizef);
  if (hdata->recordForcy == NULL) throw MDExcptFailedMallocOnHost("hdata->recordForcy", sizef);
  hdata->recordForcz = (ScalorType *) malloc (sizef);
  if (hdata->recordForcz == NULL) throw MDExcptFailedMallocOnHost("hdata->recordForcz", sizef);
...
__host__ void lazyInitHostMDData (HostMDData * hdata)
{
  ...
  for (IndexType i = 0; i < hdata->numAtom; ++i){
    ...
    hdata->recordForcx[i] = 0.;
    hdata->recordForcy[i] = 0.;
    hdata->recordForcz[i] = 0.;
...
__host__ void destroyHostMDData (HostMDData * hdata)
{
  ...
  freeAPointer ((void**)&hdata->recordForcx);
  freeAPointer ((void**)&hdata->recordForcy);
  freeAPointer ((void**)&hdata->recordForcz);
				     ...
				     HostMDData * hdata)
{
  ...
  cudaMemcpy (hdata->recordForcx, ddata->recordForcx, sizef, cudaMemcpyDeviceToHost);
  cudaMemcpy (hdata->recordForcy, ddata->recordForcy, sizef, cudaMemcpyDeviceToHost);
  cudaMemcpy (hdata->recordForcz, ddata->recordForcz, sizef, cudaMemcpyDeviceToHost);
  checkCUDAError ("cpyDeviceMDDataToHost recordForc");
...
__host__ void cpyHostMDDataToDevice (const HostMDData * hdata, DeviceMDData * ddata)
{
  ...
  cudaMemcpy (ddata->recordForcx, hdata->recordForcx, sizef, cudaMemcpyHostToDevice);
  cudaMemcpy (ddata->recordForcy, hdata->recordForcy, sizef, cudaMemcpyHostToDevice);
  cudaMemcpy (ddata->recordForcz, hdata->recordForcz, sizef, cudaMemcpyHostToDevice);
  checkCUDAError ("cpyHostMDDataToDevice recordForc");
				       ...
				       DeviceMDData * ddata)
{
  ...
  cudaMemcpy (ddata->recordForcx, hdata->recordForcx, sizef, cudaMemcpyDeviceToDevice);
  cudaMemcpy (ddata->recordForcy, hdata->recordForcy, sizef, cudaMemcpyDeviceToDevice);
  cudaMemcpy (ddata->recordForcz, hdata->recordForcz, sizef, cudaMemcpyDeviceToDevice);
  checkCUDAError ("cpyDeviceMDDataToDevice recordForc");
				...
				DeviceMDData * ddata)
{
  ...
  cudaMalloc ((void**) &ddata->recordForcx, sizef);
  cudaMalloc ((void**) &ddata->recordForcy, sizef);
  cudaMalloc ((void**) &ddata->recordForcz, sizef);
  checkCUDAError ("initDeviceMDData recordForc");
...
__host__ void destroyDeviceMDData (DeviceMDData * ddata)
{
  if (ddata->malloced){
    ...
    cudaFree (ddata->recordForcx);
    cudaFree (ddata->recordForcy);
    cudaFree (ddata->recordForcz);
			...
			const IndexType indx2)
{
  ...
  ddata2->recordForcx[indx2] = ddata1->recordForcx[indx1];
  ddata2->recordForcy[indx2] = ddata1->recordForcy[indx1];
  ddata2->recordForcz[indx2] = ddata1->recordForcz[indx1];
