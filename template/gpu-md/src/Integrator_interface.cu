...
Langevin_BAOAB::~Langevin_BAOAB()
{
  free();
}

static void checkCurandReturn (const curandStatus_t & rvalue)
{
  if (rvalue != CURAND_STATUS_SUCCESS) {
    printf ("Langevin_BAOAB::error of curand\n");
    exit (1);
  }
}

void Langevin_BAOAB::free ()
{
  if (initilized){
    cudaFree (random_x);
    cudaFree (random_y);
    cudaFree (random_z);
    checkCUDAError ("Langevin_BAOAB::free free random");
    curandStatus_t rvalue;
    rvalue = curandDestroyGenerator (gen_x);
    checkCurandReturn (rvalue);
    rvalue = curandDestroyGenerator (gen_y);
    checkCurandReturn (rvalue);
    rvalue = curandDestroyGenerator (gen_z);
    checkCurandReturn (rvalue);
  }
}


void Langevin_BAOAB::reinit (const MDSystem & sys,
			     const ScalorType & dt_,
			     const ScalorType & gamma_,
			     const ScalorType & T0_,
			     const TypeType & t0_,
			     const TypeType & t1_,
			     const IndexType & NThread,
			     unsigned long long seed)
{
  ...
  t0 = t0_;
  t1 = t1_;
  sigma = sqrt (2. * gamma * T0);
  pref0 = exp(- gamma * dt);
  pref1 = sigma / sqrt(2. * gamma) * sqrt(1. - exp (-2. * gamma * dt));
  // invsqrt3 = 1./sqrt(3.);
  invsqrt3 = 1.;
  
  cudaMalloc ((void**)&random_x, nrandom * sizeof(ScalorType));
  checkCUDAError ("Langevin_BAOAB::malloc random");
  cudaMalloc ((void**)&random_y, nrandom * sizeof(ScalorType));
  checkCUDAError ("Langevin_BAOAB::malloc random");
  cudaMalloc ((void**)&random_z, nrandom * sizeof(ScalorType));  
  checkCUDAError ("Langevin_BAOAB::malloc random");

  curandStatus_t rvalue;
  rvalue = curandCreateGenerator (&gen_x, CURAND_RNG_PSEUDO_DEFAULT);
  checkCurandReturn (rvalue);
  rvalue = curandCreateGenerator (&gen_y, CURAND_RNG_PSEUDO_DEFAULT);
  checkCurandReturn (rvalue);
  rvalue = curandCreateGenerator (&gen_z, CURAND_RNG_PSEUDO_DEFAULT);
  checkCurandReturn (rvalue);
  rvalue = curandSetPseudoRandomGeneratorSeed (gen_x, seed+0);
  checkCurandReturn (rvalue);
  rvalue = curandSetPseudoRandomGeneratorSeed (gen_y, seed+1);
  checkCurandReturn (rvalue);
  rvalue = curandSetPseudoRandomGeneratorSeed (gen_z, seed+2);
  checkCurandReturn (rvalue);

  rvalue = curandGenerateNormal(gen_x, random_x, nrandom, 0, invsqrt3);
  checkCurandReturn (rvalue);
  rvalue = curandGenerateNormal(gen_y, random_y, nrandom, 0, invsqrt3);
  checkCurandReturn (rvalue);
  rvalue = curandGenerateNormal(gen_z, random_z, nrandom, 0, invsqrt3);
  checkCurandReturn (rvalue);
...
void Langevin_BAOAB::stepAOA (MDSystem & sys,
			      MDTimer * timer)
{
  if (timer != NULL) timer->tic(mdTimeIntegrator);
  curandStatus_t rvalue;
  ...
  Langevin_BAOAB_partAOA
	  ...
	  dt);
  checkCUDAError ("Langevin_BAOAB::step1");
  if (timer != NULL) timer->toc(mdTimeIntegrator);
}

void Langevin_BAOAB::stepB (MDSystem & sys,
			    MDTimer * timer)
{
  if (timer != NULL) timer->tic(mdTimeIntegrator);
  Langevin_BAOAB_partB
      <<<atomGridDim, myBlockDim>>> (
	  sys.ddata.numAtom, sys.ddata.massi,
	  sys.ddata.velox, sys.ddata.veloy, sys.ddata.veloz,
	  sys.ddata.forcx, sys.ddata.forcy, sys.ddata.forcz,
	  dt);
  checkCUDAError ("Langevin_BAOAB::stepB");
  if (timer != NULL) timer->toc(mdTimeIntegrator);
}
			    ...
			    MDTimer * timer)
{
  ...
  Langevin_BAOAB_partB
	  ...
	  sum_kyy.buff,
	  sum_kzz.buff);
  checkCUDAError ("Langevin_BAOAB::stepB (with statistic)");
  sum_kxx.sumBuffAdd (st.ddata, mdStatisticKineticEnergyXX, 0);
  sum_kyy.sumBuffAdd (st.ddata, mdStatisticKineticEnergyYY, 0);
  sum_kzz.sumBuffAdd (st.ddata, mdStatisticKineticEnergyZZ, 0);
  if (timer != NULL) timer->toc(mdTimeIntegrator);
