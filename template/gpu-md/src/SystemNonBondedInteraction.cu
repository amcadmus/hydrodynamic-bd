...
reinit (const Topology::System & sysTop)
{
  ...
  for (unsigned i = 0; i < sysTop.nonBondedInteractions.size(); ++i){
    ...
    add (sysTop.nonBondedInteractions[i].typeOfAtom0(),
	 ...
	 printf ("# added atom %d and %d, inter type: %d\n",
	 sysTop.nonBondedInteractions[i].typeOfAtom0(),
sysTop.nonBondedInteractions[i].typeOfAtom1(),
sysTop.nonBondedInteractions[i].typeOfInter());
  }
  build ();
...
printTable () const
{
  if (typeMat.size() != numAtomTypes){
    printf ("error: numAtomTypes is not equal to typeMat.size()\n");
    exit (1);
  }
  for (unsigned ii = 0; ii < numAtomTypes; ++ii){
    for (unsigned jj = 0; jj < numAtomTypes; ++jj){
       IndexType i, j;
       ii <= jj ? (i = ii, j = jj) : (i = jj, j = ii);
       IndexType idx = i * numAtomTypes + j - ((i*(i+1)) >> 1);
       if (idx >= interactionTableSize()){
	 printf ("error: wrong interaction table size reached\n");
	 exit(1);
       }
       printf ("# atom type %03d interacts with type %03d, force idx: %d\n", ii, jj, interactionTable()[idx]);
    }
  }
  //   if (typeMat[ii].size() != numAtomTypes){
  //     printf ("error: numAtomTypes is not equal to typeMat[%d].size()\n", ii);
  //   }
  //   for (unsigned jj = 0; jj < numAtomTypes; ++jj){
  //     printf ("atom type %03d with type %03d, force type: %d\n", ii, jj, typeMat[ii][jj]);
  //   }
  // }
}
